"""Profile Warp native env: per-phase timing breakdown."""
...
env = WarpEnv(mj_model, n_envs=N, naconmax=jax_env._naconmax, njmax=jax_env._njmax, sim_dt=SIM_DT, ctrl_dt=CTRL_DT)
...
_ = renderer.render(mw_model=env.wm, mw_data=env.wd)
...
print(f"\nWarp native profiling {STEPS} steps, {N} envs, n_substeps={env.n_substeps}:\n")
...
print(f"  {'total':10s}: {total:7.1f} ms/step  ({N/total*1000:.1f} env-steps/s)")
