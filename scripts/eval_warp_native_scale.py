"""Full eval with Warp native at scale. 600 steps, N envs, configurable buffers."""
...
parser.add_argument("--ckpt", type=str, default="checkpoints/task0_model_50ep.pth")
args = parser.parse_args()

N = args.n_envs; STEPS = args.steps
...
with initialize_config_dir(version_base=None, config_dir=cd):
    cfg = compose(config_name="config", overrides=[f"seed={args.seed}","benchmark_name=libero_spatial",
...
print(f"N={N}, seed={args.seed}, steps={STEPS}", flush=True)
print(f"VRAM after init: {torch.cuda.mem_get_info(0)[0]/1e9:.2f} GB free", flush=True)
...
for s in range(STEPS):
    ...
    if (s+1) % 50 == 0:
        ...
        print(f"  step {s+1}: success={ns}/{N} ({ns/N*100:.1f}%) elapsed={elapsed:.1f}s", flush=True)
        if all(done): break

n_succ = sum(1 for i in range(N) if done[i] and steps_arr[i] < STEPS)
total_time = time.time() - t0
print(f"\nSuccess: {n_succ}/{N} = {n_succ/N*100:.1f}%")
print(f"Time: {total_time:.1f}s")
print(f"Throughput: {N*STEPS/total_time:.0f} env-steps/s")